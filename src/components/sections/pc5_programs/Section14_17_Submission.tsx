...
export const Section14_LessonsLearned: React.FC = () => {
  ...
  const data = formData.pc5p.s14;

  const handleUpdate = (updates: any) => {
    updateSection('pc5p', { s14: { ...data, ...updates } });
  };

  const fields = [
    { key: 'operation', label: 'Operation', placeholder: 'Lessons learned regarding program operations...' },
  ...
  return (
    <div className="card">
      <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'hsl(var(--primary))', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '2rem' }}>Lessons Learned during Program Life Cycle</h3>
...
export const Section15_ManagementChanges: React.FC = () => {
  const { formData, updateSection } = useForm();
  const data = formData.pc5p.s15;

  const handleUpdate = (updates: any) => {
    updateSection('pc5p', { s15: { ...data, ...updates } });
  };
    ...
    <div className="card">
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h4 style={{ fontSize: '0.875rem', fontWeight: 700, color: 'hsl(var(--primary))' }}>Changes in Program Management</h4>
            <p style={{ fontSize: '0.75rem', color: 'hsl(var(--text-muted))' }}>Has there been any change in the program management during the reporting period?</p>
          </div>
...
export const Section16_Suggestions: React.FC = () => {
  const { formData, updateSection } = useForm();
  const data = formData.pc5p.s16;

  const handleUpdate = (updates: any) => {
    updateSection('pc5p', { s16: { ...data, ...updates } });
  };
  ...
  return (
    <div className="card">
      ...
      <textarea 
        ...
        placeholder="Provide comprehensive suggestions for institutional, technical, or financial improvements based on the program's performance..."
...
export const Section17_Submission: React.FC = () => {
  const { formData, updateSection } = useForm();
  const data = formData.pc5p.submission;

  const handleUpdate = (updates: any) => {
    updateSection('pc5p', { submission: { ...data, ...updates } });
  };
