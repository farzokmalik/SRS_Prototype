...
export const Section11_Marketing: React.FC = () => {
  ...
  const data = formData.pc5p.s11;

  const handleUpdate = (updates: any) => {
    updateSection('pc5p', { s11: { ...data, ...updates } });
  };
...
export const Section12_Maintenance: React.FC = () => {
  const { formData, updateSection } = useForm();
  const data = formData.pc5p.s12;

  const handleUpdate = (updates: any) => {
    updateSection('pc5p', { s12: { ...data, ...updates } });
  };
...
export const Section13_OutputTargets: React.FC = () => {
  const { formData, updateSection } = useForm();
  const data = formData.pc5p.s13;

  const handleUpdate = (updates: any) => {
    updateSection('pc5p', { s13: { ...data, ...updates } });
  };
  ...
  return (
    <div className="card">
      <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
        <div>
          <h4 style={{ fontSize: '0.875rem', fontWeight: 700, marginBottom: '0.25rem', color: 'hsl(var(--primary))' }}>Program Output Targets Achievement</h4>
