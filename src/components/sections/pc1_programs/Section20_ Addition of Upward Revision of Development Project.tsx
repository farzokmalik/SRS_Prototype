...
export const Section2_RevisionHistory: React.FC = () => {
  ...
  const currentProposal = data.currentProposal || {
    revisionRequiredDueTo: [
      ...
      { reason: 'Increase in scope of the program', yesNo: 'No', costImpact: '' },
      { reason: 'Both revision in MRS rates and increase in scope of the program', yesNo: 'No', costImpact: '' }
    ],
    proposedRevisedCost: ''
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                ...
                <tr key={idx}>
                  ...
                  <td style={{ verticalAlign: 'middle' }}>
                    <div style={{ display: 'flex', justifyContent: 'center' }}>
                      <button type="button" onClick={() => removeRow('history', history, idx)} className="btn-icon-danger">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </td>
      ...
      <div className="card shadow-sm">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid hsl(var(--border))', paddingBottom: '1rem' }}>
          <h3 style={{ fontSize: '1.125rem', fontWeight: 600 }}>
            Year Wise Financial Progress (Program)
          </h3>
            ...
            <tbody>
              {financialProgress.map((row: any, idx: number) => (
                <tr key={idx}>
                  ...
                  <td style={{ verticalAlign: 'middle' }}>
                    <div style={{ display: 'flex', justifyContent: 'center' }}>
                      <button type="button" onClick={() => removeRow('financialProgress', financialProgress, idx)} className="btn-icon-danger" style={{ background: 'transparent', border: 'none', padding: '4px', cursor: 'pointer' }}>
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </td>
      ...
      <div className="card shadow-sm">
        <div style={{ borderBottom: '1px solid hsl(var(--border))', paddingBottom: '1rem', marginBottom: '1.5rem' }}>
          <h3 style={{ fontSize: '1.125rem', fontWeight: 600 }}>
            Current Program Proposal Details
          </h3>
        </div>
        <div style={{ overflowX: 'auto', padding: '0 1rem' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                <th style={{ padding: '1rem', borderBottom: '1px solid hsl(var(--border))', textAlign: 'center', fontSize: '0.9rem', fontWeight: 700, color: 'hsl(var(--text-main) / 0.8)' }}>
      ...
      <div className="card shadow-sm">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', borderBottom: '1px solid hsl(var(--border))', paddingBottom: '1rem' }}>
          <h3 style={{ fontSize: '1.125rem', fontWeight: 600 }}>
            Approved vs Revised Scope (Components)
          </h3>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            {scopeComparison.map((row: any, idx: number) => (
              <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1.2fr 48px', gap: '1.5rem', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center' }}>
                  <button type="button" onClick={() => removeRow('scopeComparison', scopeComparison, idx)} className="btn-icon-danger">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
      ...
      <div className="card shadow-sm">
        <h3 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '1.25rem' }}>Program Details in Quantitative / Physical Terms</h3>
        <RTEditor 
          label="" 
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: '800px' }}>
            {futureFunding.map((row: any, idx: number) => (
              <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 48px', gap: '1.5rem', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'center' }}>
                  <button type="button" onClick={() => removeRow('futureFunding', futureFunding, idx)} className="btn-icon-danger">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
      ...
      {/* 8. Annual Income */}
      <div className="card shadow-sm">
        <h3 style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '1.25rem' }}>Annual Income from Program</h3>
        <RTEditor 
          label="" 
      ...
      <div className="card shadow-sm" style={{ borderTop: '4px solid hsl(var(--accent))' }}>
        ...
        <RTEditor 
          label="Proposed Program Recommendation / Approval Notes" 
          value={data.recommendation} 
          onChange={(val) => handleUpdate({ recommendation: val })} 
        />
        <div style={{ marginTop: '2rem' }}>
          <FileUpload 
            label="Program Revision Attachments" 
            ...
            description="Upload any other supporting documents (Program Briefs, PC-II data, etc.)"
          />
        </div>
      </div>
    </div>
