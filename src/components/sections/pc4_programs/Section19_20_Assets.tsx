...
export const Section19_PDHistory: React.FC = () => {
  ...
  const data = formData.pc4p.s19;

  const handleUpdate = (updates: any) => {
    updateSection('pc4p', { s19: { ...data, ...updates } });
  };
    ...
    <div className="card">
      ...
      <div className="table-responsive">
        <table className="table" style={{ borderCollapse: 'separate', borderSpacing: '1rem 0.75rem', marginTop: '-0.75rem', marginLeft: '-1rem', width: 'calc(100% + 1rem)' }}>
          <thead>
            <tr>
              <th style={{ width: '40%', background: 'transparent', border: 'none', color: 'hsl(var(--text-muted))', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', paddingBottom: '0.5rem', textAlign: 'left' }}>Name of Program Director</th>
          ...
          <tbody>
            {data.pdHistory.length === 0 && (
              <tr><td colSpan={4} style={{ textAlign: 'center', padding: '3rem', color: 'hsl(var(--text-muted))', background: 'hsl(var(--bg-main))', borderRadius: 'var(--radius-md)', border: '1px dashed hsl(var(--border))' }}>No Program Director history recorded</td></tr>
            )}
...
export const Section20_Assets: React.FC = () => {
  const { formData, updateSection } = useForm();
  const data = formData.pc4p.s20;

  const handleUpdate = (updates: any) => {
    updateSection('pc4p', { s20: { ...data, ...updates } });
  };
