...
export const Section21_ImpactEvaluation: React.FC = () => {
  ...
  const data = formData.pc4p.s21;

  const handleUpdate = (updates: any) => {
    updateSection('pc4p', { s21: { ...data, ...updates } });
  };
        ...
        {impactFields.map((field) => (
          <div key={field.id}>
            ...
            <textarea 
              ...
              placeholder={`Describe the ${field.label.toLowerCase()} of the program...`}
...
export const Section22_ImpactAnalysis: React.FC = () => {
  const { formData, updateSection } = useForm();
  const data = formData.pc4p.s22;

  const handleUpdate = (updates: any) => {
    updateSection('pc4p', { s22: { ...data, ...updates } });
  };
    ...
    <div className="card">
      <div style={{}}>
        <p style={{ fontSize: '0.875rem', color: 'hsl(var(--text-muted))', marginBottom: '1rem' }}>
          Provide a detailed analysis of the program's overall impact on the target population and sector.
        </p>
        <textarea 
          className="input" 
...
export const Section23_EconomicAnalysis: React.FC = () => {
  const { formData, updateSection } = useForm();
  const data = formData.pc4p.s23;

  const handleUpdate = (updates: any) => {
    updateSection('pc4p', { s23: { ...data, ...updates } });
  };
