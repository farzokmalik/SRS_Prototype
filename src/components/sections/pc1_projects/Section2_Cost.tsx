      ...
      <div className="card">
        ...
        {data.foreignFunding === 'Yes' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginTop: '1rem', padding: '1rem', background: '#f8f9fa', borderRadius: 'var(--radius-md)' }}>
            <InputField label="[PC-I-1.15] Foreign Exchange Component (FEC)" value={data.fec} onChange={(e) => handleUpdate({ fec: e.target.value })} />
            ...
            <InputField label="[PC-I-1.19] Source of Currency" value={data.source} onChange={(e) => handleUpdate({ source: e.target.value })} />
            <InputField label="[PC-I-1.20] Exchange Rate" value={data.exchangeRate} onChange={(e) => handleUpdate({ exchangeRate: e.target.value })} />
          </div>
        )}
      </div>
