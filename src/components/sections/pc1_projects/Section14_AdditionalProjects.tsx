    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      <div className="card">
        ...
        {data.filter === 'Existing Scheme' && (
          <div style={{ marginTop: '1rem', display: 'flex', gap: '0.5rem' }}>
             <div style={{ flex: 1, maxWidth: '300px' }}>
               <InputField label="[PC-I-14.2] Search GS No" placeholder="Enter GS No" value={data.gsNo} onChange={(e) => handleUpdate({ gsNo: e.target.value })} />
             </div>
        ...
        {data.filter === 'New Scheme' && (
          <div style={{ marginTop: '1rem', display: 'flex', gap: '0.5rem' }}>
             <div style={{ flex: 1, maxWidth: '300px' }}>
               <InputField label="[PC-I-14.2] Search Scheme" placeholder="Enter Scheme Name or Keyword" value={data.newSchemeSearch || ''} onChange={(e) => handleUpdate({ newSchemeSearch: e.target.value })} />
             </div>
             <button className="btn btn-primary" style={{ height: '42px', marginTop: '28px' }}><Search size={18} /> Search</button>
          </div>
        )}
      ...
      <div className="card">
        <RTEditor label="[PC-I-14.3] Additional Project Decision Requirement Information" value={data.description} onChange={(val) => handleUpdate({ description: val })} />
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
        <div className="card">
          <FileUpload 
            label="[PC-I-14.4] Attachments"
