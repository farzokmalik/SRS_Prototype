...
export const Section6_ProjectIncome: React.FC = () => {
  const { formData, updateSection } = useForm();
  const data = formData.pc5.s6;

  const handleUpdate = (updates: any) => {
    updateSection('pc5', { s6: [...updates] });
  };
  ...
  return (
    <div className="card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
        <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'hsl(var(--primary))', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Project Income Analysis</h3>
        <button className="btn btn-secondary" onClick={addItem} style={{ padding: '0.5rem 1rem', fontSize: '0.8125rem' }}>
...
export const Section7_EconomicBenefits: React.FC = () => {
  const { formData, updateSection } = useForm();
  const data = formData.pc5.s7;

  const handleUpdate = (updates: any) => {
    updateSection('pc5', { s7: [...updates] });
  };
