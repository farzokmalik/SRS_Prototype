...
export const Section3_Sponsoring: React.FC = () => {
  ...
  return <AgencySection title="Sponsoring Ministry" data={data} onUpdate={(updates) => updateSection('pc4', { s3: { ...data, ...updates } })} />;
};

export const Section4_Executing: React.FC = () => {
  const { formData, updateSection } = useForm();
  const data = formData.pc4.s4;
  return <AgencySection title="Executing Agency" data={data} onUpdate={(updates) => updateSection('pc4', { s4: { ...data, ...updates } })} />;
};

export const Section5_OM: React.FC = () => {
  const { formData, updateSection } = useForm();
  const data = formData.pc4.s5;
  return <AgencySection title="O&M Agency" data={data} onUpdate={(updates) => updateSection('pc4', { s5: { ...data, ...updates } })} />;
};
