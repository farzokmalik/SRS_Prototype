...
export const Section16_ClimateCoBenefits: React.FC = () => {
  ...
  const data = formData.pc4.s16;

  const handleUpdate = (updates: any) => {
    updateSection('pc4', { s16: { ...data, ...updates } });
  };
...
export const Section17_ProjectIncome: React.FC = () => {
  const { formData, updateSection } = useForm();
  const data = formData.pc4.s17;

  const handleUpdate = (updates: any) => {
    updateSection('pc4', { s17: { ...data, ...updates } });
  };
  ...
  return (
    <div className="card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h3 style={{ fontSize: '1.25rem', fontWeight: 700, color: 'hsl(var(--primary))', margin: 0 }}>Project Income / Revenue Generation</h3>
